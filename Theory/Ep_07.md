...
Using anchor tag `<a href={"/"}> Home <a>` we achieve `Server-side routing or rendering (SSR)`.

Using Link tag of `react-router-dom` `<Link to={"/"}> Home </Link>` we can achieve `Client-side routing or rendering (CSR)`.
